...
import { Add_To_Review } from '../../../api/category_product/Category_Product'
...
const Add_Product_Review = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const [is_Loading, set_Is_Loading] = useState(false)
    const { order_details_result } = location.state || {};
    console.log("order_details_result", order_details_result)
    ...
    const Handle_Add_to_review = async () => {
        set_Is_Loading(true);
        const data = {
            product_id: order_details_result,
            rating: rating,
            review: review
        }
        try {
            ...
            const response = await Add_To_Review(data, { Authorization: `Bearer ${token}` })
                                        ...
                                        <div className="gi-login-wrapper my-[0] mx-auto">
                                            <div className="gi-login-container">
                                                <div className="gi-login-form">
                                                    <form action="#" method="post" className="flex flex-col" onSubmit={Handle_Add_to_review}>
