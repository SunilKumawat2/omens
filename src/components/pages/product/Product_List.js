...
const Product_List = () => {
    ...
    const [is_Open_Certificate, set_Is_Open_Certificate] = useState(false);

    const [is_Open_Color, set_Is_Open_Color] = useState(false);
    const [is_Open_Composition, set_Is_Open_Composition] = useState(false);
    ...
    const [video, set_video] = useState("https://rasatva.apponedemo.top/omens/public/uploads/product/videos/19056.mp4")
    ...
    const handlePlay_Video = (video_url) => {
        set_video(`${IMG_BASE_URL}${video_url}`)
        set_show_video(true)
    }
    ...
    const toggle_Certificate_accordion = () => {
        set_Is_Open_Certificate(!is_Open_Certificate);
    };
    const toggle_color_accordion = () => {
        set_Is_Open_Color(!is_Open_Color);
    };
    const toggle_composition_accordion = () => {
        set_Is_Open_Composition(!is_Open_Composition);
    };
    ...
    // <------------ handle Certifiate Checkbox Change ----------->
    const handle_certificate_Checkbox_Change = (CertificateName) => {
        setSelectedCertificate((prevSelected) =>
            prevSelected?.includes(CertificateName)
                ? prevSelected?.filter((name) => name !== CertificateName)
                : [...prevSelected, CertificateName]
        );
    };
    const selectedCertificateString = selectedCertificate.join(',');
    // console.log("selected_certificate:",selectedCertificate);
    // console.log("selected_shape:",selectedShape);
    ...
    // <------------ handle Composition Checkbox Change ----------->
    const handle_composition_Checkbox_Change = (CompositionName) => {
        setSelectedComposition((prevSelected) =>
            prevSelected?.includes(CompositionName)
                ? prevSelected?.filter((name) => name !== CompositionName)
                : [...prevSelected, CompositionName]
        );
    };
    const selectedCompositionString = selectedComposition.join(',');


    // <------------ handle Treatment Checkbox Change ----------->
                ...
                const response = await Get_Product_List_Category_wise(
                    {
                        ...
                        origin_id: selectedOriginsString,
                        shape_id: selectedShapeString,
                        cut_id: selectedCutString,
                        certificate_id: selectedCertificateString,
                        color_id: selectedColorString,
                        composition_id: selectedCompositionString,
                        treatment_id: selectedTreatmentString,
                        min_amount: minPrice,
                        max_amount: maxPrice
                    },
    ...
    }, [Get_user_is_active, category_id, subcategory_id, selectedOriginsString, selectedShapeString, selectedCutString, selectedCertificateString, selectedColorString, selectedCompositionString, selectedTreatmentString, minPrice, maxPrice]);
                                    ...
                                    <div className="gi-shop-sidebar md:sticky top-[24px] min-[992px]:w-[25%] min-[768px]:w-full w-full max-[991px]:mt-[30px] px-[12px]">
                                        <div id="shop_sidebar">
                                            <div className="gi-sidebar-wrap p-[15px] rounded-[5px] shadow-lg">
                                                                        ...
                                                                        return (
                                                                            <li>
                                                                                <div className="gi-sidebar-block-item py-[10px] relative flex flex-row">
                                                                                    ...
                                                                                    <span className={`checked absolute top-1/2 left-0 h-4 w-4 bg-white border border-gray-200 transition-transform transform -translate-y-1/2 rounded ${selectedCut.includes(cut_result?.id) ? "bg-red-500 border-red-500" : ""}`}></span>
                                                                                </div>
                                                                            </li>
                                                                        )
                                                                    })
                                                ...
                                                {/* <!-- Sidebar Certificate Block --> */}
                                                <div className="gi-sidebar-block mb-[15px] cursor-pointer">
                                                    <div className="gi-sb-title border-b-[1px] border-solid border-[#eee] pb-[15px]">
                                                        <span className={`transform transition-transform ${is_Open_Certificate ? "rotate-180" : ""}`}></span>
                                                        <h3 className="gi-sidebar-title font-semibold tracking-[0] relative text-[#4b5966] w-full
                                                                 flex justify-between font-Poppins text-[14px] leading-[1.2]" onClick={toggle_Certificate_accordion}>Certificate <i class="gicon gi-angle-down"></i></h3>
                                                    </div>
                                                    {is_Open_Certificate && (
                                                        <div className="gi-sb-block-content mt-[15px]">
                                                            <ul>
                                                                {
                                                                    products?.filter_by?.certificate
                                                                        ?.map((certificate_result) => {
                                                                            return (
                                                                                <li>
                                                                                    <div className="gi-sidebar-block-item py-[10px] relative flex flex-row">
                                                                                        <input
                                                                                            type="checkbox"
                                                                                            className="w-full h-full absolute opacity-0 cursor-pointer z-10"
                                                                                            checked={selectedCertificate.includes(certificate_result?.id)}
                                                                                            onChange={() => handle_certificate_Checkbox_Change(certificate_result?.id)}
                                                                                        />
                                                                                        <p className="w-full text-[#777] text-[14px] mt-[0] leading-[20px] font-normal capitalize cursor-pointer flex justify-between pl-[30px]">
                                                                                            <span className="flex">{certificate_result?.name}</span>
                                                                                        </p>
                                                                                        <span className={`checked absolute top-1/2 left-0 h-4 w-4 bg-white border border-gray-200 transition-transform transform -translate-y-1/2 rounded ${selectedCertificate.includes(certificate_result?.id) ? "bg-red-500 border-red-500" : ""}`}></span>
                                                                                    </div>
                                                                                </li>
                                                                            )
                                                                        })
                                                ...
                                                {/* <!-- Sidebar composition item --> */}
                                                <div className="gi-sidebar-block mb-[15px] cursor-pointer">
                                                    <div className="gi-sb-title border-b-[1px] border-solid border-[#eee] pb-[15px]">
                                                        <span className={`transform transition-transform ${is_Open_Composition ? "rotate-180" : ""}`}></span>
                                                        <h3 className="gi-sidebar-title font-semibold tracking-[0] relative text-[#4b5966] w-full
                                                                 flex justify-between font-Poppins text-[14px] leading-[1.2]" onClick={toggle_composition_accordion}>Composition <i class="gicon gi-angle-down"></i></h3>
                                                    </div>
                                                    {is_Open_Composition && (
                                                        <div className="gi-sb-block-content mt-[15px]">
                                                            <ul>
                                                                {
                                                                    products?.filter_by?.composition
                                                                        ?.map((composition_result) => {
                                                                            return (
                                                                                <li>
                                                                                    <div className="gi-sidebar-block-item py-[10px] relative flex flex-row">
                                                                                        <input
                                                                                            type="checkbox"
                                                                                            className="w-full h-full absolute opacity-0 cursor-pointer z-10"
                                                                                            checked={selectedComposition.includes(composition_result?.id)}
                                                                                            onChange={() => handle_composition_Checkbox_Change(composition_result?.id)}
                                                                                        />
                                                                                        <p className="w-full text-[#777] text-[14px] mt-[0] leading-[20px] font-normal capitalize cursor-pointer flex justify-between pl-[30px]">
                                                                                            <span className="flex">{composition_result?.name}</span>
                                                                                        </p>
                                                                                        <span className={`checked absolute top-1/2 left-0 h-4 w-4 bg-white border border-gray-200 transition-transform transform -translate-y-1/2 rounded ${selectedComposition.includes(composition_result?.id) ? "bg-red-500 border-red-500" : ""}`}></span>
                                                                                    </div>
                                                                                </li>
                                                                            )
                                                                        })
                                                                                ...
                                                                                <div className="gi-product-inner transition-all duration-[0.3s] ease-in-out overflow-hidden rounded-[5px] shadow-xl">
                                                                                    <div className="gi-pro-image-outer transition-all duration-[0.3s] delay-[0s] ease z-[11] relative">
                                                                                        <div className="gi-pro-image overflow-hidden">
                                                                                            ...
                                                                                            {
                                                                                                product_list_result?.single_video?.video_url != null && (
                                                                                                    <span onClick={() => handlePlay_Video(product_list_result?.product_images[1]?.video_url)} className="flags flex flex-col h-8 w-8 bg-white items-center justify-center border rounded-full hover:bg-gray-200 cursor-pointer uppercase absolute top-[5px] right-[60px] z-[2]">
                                                                                                        <i
                                                                                            ...
                                                                                            <a href="#" className="image productimg7 relative block overflow-hidden pointer-events-none">
                                                                                                <img className="first_pro main-image object-cover h-[250px] transition-all duration-[0.3s] ease delay-[0s] "
                                                                                                    src={`${IMG_BASE_URL}${product_list_result?.product_images[0]?.image_url}`} alt="Product" />


                                                                                                {/* {product_list_result?.product_images?.length > 1 && */}
                                                                                                <img className="first_pro_hover main-image object-cover h-[250px] transition-all duration-[0.3s] ease delay-[0s]  " src={`${IMG_BASE_URL}${product_list_result?.product_images[1]?.image_url}`} alt="" />
                                                                                                {/* } */}
                                                                                    ...
                                                                                    {/* product videos */}
                                                                                    {
                                                                                        show_video == true && (
                                                                                            <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/10 backdrop-blur-[2px]">
                                                                                                {/* Close Button */}
                                                                                                <button
                                                                                                    onClick={() => set_show_video(false)}
                                                                                                    className="absolute top-90 right-90 text-white bg-black bg-opacity-60 rounded-full w-8 h-8 flex items-center justify-center text-xl hover:bg-opacity-80 z-30"
                                                                                                >
                                                                                                    &times;
                                                                                                </button>

                                                                                                {/* Video Modal */}
                                                                                                <div className="w-full max-w-[500px] h-auto rounded-lg overflow-hidden shadow-lg relative">
                                                                                                    <video
                                                                                                        autoPlay
                                                                                                        loop
                                                                                                        muted
