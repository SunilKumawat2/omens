...
(function ($) {
  ...
  /*--------------------- astrologer_gallery ---------------------- */
  $('.astrologer_gallery').owlCarousel({
    margin: 24,
    loop: true,
    dots: true,
    nav: false,
...
(function ($) {
  ...
  $(document).ready(function () {
      ...
      $('.modal-link').on('click', function (e) {
          e.preventDefault();
          var targetModal = $(this).data('modal-id'); // Get the modal ID from the data attribute
          $('#' + targetModal).addClass('open'); // Add the 'open' class to the modal
          $('body').addClass('modal-open'); // Prevent body scrolling
      });

      // Close the modal when the close button is clicked
      $('.close-modal').on('click', function () {
          $(this).closest('.custom-modal').removeClass('open'); // Remove the 'open' class from the closest modal
